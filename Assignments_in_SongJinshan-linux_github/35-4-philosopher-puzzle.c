...
void *a_philosopher_s_action(void *arg)
{
	int index = *((int *) arg);
...
int main(void)
{
	...
	for (i = 0; i < NUM; i++) 
	{
		...
		pthread_create(philosophers+i, NULL, a_philosopher_s_action, index+i);
	}
	for (i = 0; i < NUM; i++) 
	{
...
//    index[i] = i;
//    sleep(1);
//    pthread_create(philosophers+i, NULL, a_philosopher_s_action, &i);
//    //pthread_create(philosophers+i, NULL, a_philosopher_s_action, index+i);
//  }
//  for (i = 0; i < NUM; i++) {
//    pthread_join(*(philosophers+i), NULL);
